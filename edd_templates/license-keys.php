	...
	<table id="edd_sl_license_keys" class="edd_sl_table">
		<thead>
			<tr class="edd_sl_license_row">
				...
				<th class="edd_sl_item"><?php _e( 'Item', 'abcd' ); ?></th>
				<th class="edd_sl_details"><?php _e( 'License Details', 'abcd' ); ?></th>
		...
		<?php if ( $license_keys ) : ?>
			<?php foreach ( $license_keys as $license ) : ?>
				...
				<tr class="edd_sl_license_row">
					...
					<td>
						<span class="edd_sl_status_label"><?php _e( 'Status:', 'abcd' ); ?>&nbsp;</span>
						...
						<div class="edd_sl_item_expiration">
							<span class="edd_sl_expires_label edd_sl_expiries_label"><?php 'expired' === edd_software_licensing()->get_license_status( $license->ID ) ? _e( 'Expired:', 'abcd' ) : _e( 'Expires:', 'abcd' ); ?>&nbsp;</span>
						...
						<span class="edd_sl_limit_label"><?php _e( 'Activations:', 'abcd' ); ?>&nbsp;</span>
						...
						<?php if( ! edd_software_licensing()->force_increase() ) : ?>
							<br/><a href="<?php echo esc_url( add_query_arg( array( 'license_id' => $license->ID, 'action' => 'manage_licenses', 'payment_id' => $payment_id ), get_permalink( edd_get_option( 'purchase_history_page' ) ) ) ); ?>"><?php _e( 'Manage Sites', 'abcd' ); ?></a>
						<?php endif; ?>
						<?php if( edd_sl_license_has_upgrades( $license->ID ) && 'expired' !== edd_software_licensing()->get_license_status( $license->ID ) ) : ?>
							...
							<a href="<?php echo esc_url( add_query_arg( array( 'view' => 'upgrades', 'license_id' => $license->ID, 'action' => 'manage_licenses', 'payment_id' => $payment_id ), get_permalink( edd_get_option( 'purchase_history_page' ) ) ) ); ?>"><?php _e( 'View Upgrades', 'abcd' ); ?></a>
						<?php elseif ( edd_sl_license_has_upgrades( $license->ID ) && 'expired' == edd_software_licensing()->get_license_status( $license->ID ) ) : ?>
							<span class="edd_sl_limit_sep">&nbsp;&ndash;&nbsp;</span>
							<span class="edd_sl_no_upgrades"><?php _e( 'Renew to upgrade', 'abcd' ); ?></span>
						<?php endif; ?>
						<?php if( edd_sl_renewals_allowed() ) : ?>
							<?php if( 'expired' === edd_software_licensing()->get_license_status( $license->ID ) ) : ?>
								<span class="edd_sl_key_sep">&nbsp;&ndash;&nbsp;</span>
								<a href="<?php echo edd_software_licensing()->get_renewal_url( $license->ID ); ?>" title="<?php esc_attr_e( 'Renew license', 'abcd' ); ?>"><?php _e( 'Renew license', 'abcd' ); ?></a>
							<?php elseif( ! edd_software_licensing()->is_lifetime_license( $license->ID ) && edd_software_licensing()->can_extend( $license->ID ) ) : ?>
								<span class="edd_sl_key_sep">&nbsp;&ndash;&nbsp;</span>
								<a href="<?php echo edd_software_licensing()->get_renewal_url( $license->ID ); ?>" title="<?php esc_attr_e( 'Extend license', 'abcd' ); ?>"><?php _e( 'Extend license', 'abcd' ); ?></a>
							<?php endif; ?>
						<?php endif; ?>
						<br/>
						<a class="edd_sl_purchase_number" href="<?php echo esc_url( edd_get_success_page_uri( '?payment_key=' . edd_get_payment_key( $payment_id ) ) ); ?>" title="<?php esc_attr_e( 'View Purchase Record', 'abcd' ); ?>"><?php printf( __( 'Purchase #%d', 'abcd' ), edd_get_payment_number( $payment_id ) ); ?></a>
		...
		<?php else: ?>
			<tr class="edd_sl_license_row">
				<td colspan="2"><?php _e( 'You currently have no licenses', 'abcd' ); ?></td>
			</tr>
		<?php endif; ?>
	</table>
...
<?php else : ?>
	<p class="edd-alert edd-alert-warn">
		<?php _e( 'You must be logged in to view license keys.', 'abcd' ); ?>
	</p>
